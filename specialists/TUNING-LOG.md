...
The specialist era closes here (2026-07-21). The log continues at the
repository root `TUNING-LOG.md`, which opens with the pivot's decision
record. This file stays as the specialist package's lab notebook; the
code it documents changes only for bugs, and its battery commands now
read `pypy3 -m specialists ...`.
